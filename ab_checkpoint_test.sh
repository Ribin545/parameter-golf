...
LOG="logs/ab_checkpoint_$(date +%s).txt"
mkdir -p logs
echo "=== A/B Checkpoint Test at 100k batch ===" | tee -a $LOG
echo "Timestamp: $(date)" | tee -a $LOG
echo "" | tee -a $LOG

# Variant A: Baseline (MLP+ATTN checkpointing, no multilayer)
echo "=== Variant A: Baseline (MLP=checkpoint ATTN=checkpoint MULTILAYER=0) ===" | tee -a $LOG
rm -f best_model.pt best_model.int8.ptz
export MLP_MEMORY_MODE=checkpoint
export ATTN_MEMORY_MODE=checkpoint
unset MULTILAYER_ACTIVATION_CHECKPOINT
unset MULTILAYER_ACTIVATION_CHECKPOINT_MODE
export RUN_ID=ab_a_baseline
/home/tltp2128/.venvs/pg3090/bin/python -u train_gpt.py 2>&1 | tee -a $LOG
echo "--- Variant A done ---" | tee -a $LOG
echo "" | tee -a $LOG

# Variant B: Multilayer encoder checkpointing only
echo "=== Variant B: Multilayer encoder (MLP=off ATTN=off MULTILAYER=1 mode=encoder) ===" | tee -a $LOG
rm -f best_model.pt best_model.int8.ptz
...
export RUN_ID=ab_b_encoder
/home/tltp2128/.venvs/pg3090/bin/python -u train_gpt.py 2>&1 | tee -a $LOG
echo "--- Variant B done ---" | tee -a $LOG
echo "" | tee -a $LOG

# Variant C: Multilayer full checkpointing
echo "=== Variant C: Multilayer full (MLP=off ATTN=off MULTILAYER=1 mode=full) ===" | tee -a $LOG
rm -f best_model.pt best_model.int8.ptz
export MULTILAYER_ACTIVATION_CHECKPOINT_MODE=full
export RUN_ID=ab_c_full
/home/tltp2128/.venvs/pg3090/bin/python -u train_gpt.py 2>&1 | tee -a $LOG
echo "--- Variant C done ---" | tee -a $LOG

echo "" | tee -a $LOG
echo "=== A/B Test Complete ===" | tee -a $LOG
