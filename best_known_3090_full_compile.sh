...
# Best-known 3090 config + safe full-model compile experiment.
# Use this to test whether full-model compile beats the current best-known dt.

export RUN_ID=best_known_3090_full_compile
...
# New compile experiment
export FULL_MODEL_COMPILE=1
export ENABLE_RECURRENT_TRAIN_COMPILE=1
export TORCH_COMPILE_MODE=default

python3 -u train_gpt.py 2>&1 | tee /tmp/best_known_3090_full_compile.log
echo "--- BEST KNOWN 3090 FULL COMPILE RESULT ---"
grep -E '\[stop\]|\[final\]\[quant_eval\]|step:[0-9]+ loss:' /tmp/best_known_3090_full_compile.log | tail -30