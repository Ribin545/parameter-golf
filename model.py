...
class ShellCenteringPenalty(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        # x: [B, T, d_model]
        # Penalise individual activations that collapse toward the origin
        norms = x.norm(dim=-1, p=2)  # [B, T]
        self.penalty = self.lam * (((2 * self.d_model / ((norms ** 2) + 0.1)) ** 0.5) - 1).clamp(min=0).mean()

        # Force the centroid of activations to stay near the origin
...
class Rotary(nn.Module):
    ...
    def forward(self, seq_len: int, device: torch.device, dtype: torch.dtype) -> tuple[Tensor, Tensor]:
        # No caching — trivial compute for torch.compile to fuse into CUDA graph
...
class GPT(nn.Module):
    def __init__(
        ...
        # Shell-centering penalty on embeddings (free regularisation, issue #2045)
